...
"""populate_other_brands_detail.py — Other Brands detail (Test tab Table 7, 9x5).

Same structure as populate_other_brands_detail.py. Column layout matches Table 5:
...
def find_other_brands_table(content):
    """9 rows x 5 cols, R02 col 0 == "TIDAL"."""
    for elem in content:
        if "table" not in elem:
            continue
        tbl = elem["table"]
        if tbl.get("rows", 0) != 9 or tbl.get("columns", 0) != 5:
            continue
        try:
...
def build_values_requests(packet, table, tab_id):
    ...
    for row_spec in packet["other_brands_detail"]["rows"]:
...
def main() -> int:
    p = argparse.ArgumentParser(description="Populate Other Brands detail (Test tab Table 7)")
    ...
    with open(args.packet) as f:
        packet = json.load(f)
    if "other_brands_detail" not in packet:
        print("ERROR: packet has no other_brands_detail block. "
              "Run mrp_data.py with --other-brands-raw first.", file=sys.stderr)
        return 1
    ...
    if content is None:
        ...
        return 1
    table = find_other_brands_table(content)
    if table is None:
        print(f"ERROR: Other Brands detail table (9x5, R02='TIDAL') not found in tab {args.tab}.",
              file=sys.stderr)
        return 1
