...
"""populate_square_detail.py — Square detail (Test tab Table 6, 20x5).

Same structure as populate_cash_app_detail.py. Column layout matches Table 5:
label / Actual / $ vs. Q2OL / % vs. Q2OL / YoY % Growth.

Two passes (run with a doc re-read in between):
  --pass values   populate data cells
...
VS_OL_DOLLAR_PACKET_IDX = 1
VS_OL_PCT_PACKET_IDX = 2

DELTA_DOLLAR_THRESHOLD_M = 0.5
DELTA_PCT_THRESHOLD = 0.005
DELTA_PTS_THRESHOLD = 0.005
...
def get_tab_content(doc, tab_id):
    return _search_tabs(doc.get("tabs", []), tab_id)


def _search_tabs(tabs, tab_id):
    for tab in tabs:
...
def find_square_table(content):
    """20 rows x 5 cols, R02 col 0 starts with 'New Volume Added'."""
    for elem in content:
        if "table" not in elem:
            continue
        tbl = elem["table"]
        if tbl.get("rows", 0) != 20 or tbl.get("columns", 0) != 5:
            continue
        try:
            label = _cell_text(tbl["tableRows"][2]["tableCells"][0]).strip()
        except (IndexError, KeyError):
            continue
        if label.startswith("New Volume Added"):
            return tbl
    return None


def _cell_text_runs(cell):
    runs = []
...
def build_values_requests(packet, table, tab_id):
    ...
    for row_spec in packet["square_detail"]["rows"]:
...
def color_for_dollar(value, polarity):
...
def color_for_pct(value, polarity):
...
def color_for_pts(value, polarity):
...
def main() -> int:
    p = argparse.ArgumentParser(description="Populate Square detail (Test tab Table 6)")
    ...
    with open(args.packet) as f:
        packet = json.load(f)
    if "square_detail" not in packet:
        print("ERROR: packet has no square_detail block. "
              "Run mrp_data.py with --square-raw first.", file=sys.stderr)
        return 1
    ...
    if content is None:
        ...
        return 1
    table = find_square_table(content)
    if table is None:
        print(f"ERROR: Square detail table (20x5, R02='New Volume Added*') not found in tab {args.tab}.",
              file=sys.stderr)
        return 1
